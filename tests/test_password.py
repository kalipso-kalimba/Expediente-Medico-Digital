...
def login(pw):
    ...
    r = s.post(f"{BASE}/login",
        data={"username": "doctor", "password": pw, "csrf_token": csrf},
...
csrf = link_csrf(r.text)
r = s.post(f"{BASE}/doctor/change-password", data={
    "csrf_token": csrf,
...
s2.post(f"{BASE}/doctor/change-password", data={
    "csrf_token": csrf2,
